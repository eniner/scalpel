import type { Translation, Node, SkillTreeData, Group, Sprite, TranslationFile } from './skill-tree-types'
import { requireData } from '../crystalline'
...
export const loadSkillTree = () => {
  skillTree = JSON.parse(requireData().SkillTree);
  ...
  const translationFiles = [
    requireData().StatTranslationsJSON,
    requireData().PassiveSkillStatTranslationsJSON,
    requireData().PassiveSkillAuraStatTranslationsJSON
  ];
  ...
  Object.keys(requireData().TreeToPassive).forEach((k) => {
    passiveToTree[requireData().TreeToPassive[parseInt(k)].Index] = parseInt(k);
  });
};
...
export const getStat = (id: number | string): Stat => {
  ...
  if (!(nId in statCache)) {
    statCache[nId] = requireData().GetStatByIndex(nId)!;
  }
