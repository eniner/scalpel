...
function positionAtCursor(): void {
  ...
  const x = Math.round(
    Math.max(area.x, Math.min(cursor.x - LAUNCHER_SIZE / 2, area.x + area.width - LAUNCHER_SIZE)),
  )
  const y = Math.round(
    Math.max(area.y, Math.min(cursor.y - LAUNCHER_SIZE / 2, area.y + area.height - LAUNCHER_SIZE)),
  )
